  ...
  const addExperiment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const file of newExperiment.protocolFiles) {
        const uploadResult = await uploadFileToStorage(file, experimentData.id, "protocol")
        if (uploadResult) {
                              ...
                              {experiment.protocols.map((protocol) => (
                                ...
                                >
                                  ...
                                  <span className="flex-1">
                                    {protocol.steps?.file_name || protocol.title || `Protocol ${protocol.id.slice(0, 8)}`}
                                  </span>
                                  {(protocol.steps?.file_url || protocol.file_url) && (
                                    <Button variant="ghost" size="sm" asChild>
                                      <a href={protocol.steps?.file_url || protocol.file_url} target="_blank" rel="noopener noreferrer">
