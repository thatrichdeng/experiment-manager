...
import { Plus, Search, FileText, Database, TagIcon, Calendar, User, Beaker, Microscope } from "lucide-react"
...
export default function ResearchPlatform() {
    ...
    try {
      ...
      const experimentsWithRelations = await Promise.all(
        (experimentsData || []).map(async (exp) => {
          // Fetch tags for this experiment - handle both table name formats
          let tagData = null
          try {
            // Try with hyphenated table name first
            const { data } = await supabase
              .from("experiments-tags")
              .select(`
                tags (*)
              `)
            ...
            // If that fails, try with underscore
            try {
              const { data } = await supabase
  ...
  const fetchTags = async () => {
    ...
    try {
      ...
      // Ensure each tag has required properties with defaults
  ...
  const addExperiment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newExperiment.tag_ids.length > 0) {
        ...
        let tagError = null
        try {
          const { error } = await supabase.from("experiments-tags").insert(tagInserts)
          tagError = error
        } catch (error) {
          // If that fails, try with underscore
          try {
            const { error: error2 } = await supabase.from("experiment_tags").insert(tagInserts)
            tagError = error2
          } catch (error3) {
            tagError = error3
          }
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center justify-between">
                              <Label className="text-sm font-medium flex items-center gap-2">
                                ...
                                Protocols ({experiment.protocols.length})
                              </Label>
                              <Dialog>
                                <DialogTrigger asChild>
                            ...
                            <div className="space-y-1">
                              {experiment.protocols.map((protocol) => (
                                ...
                                >
                                  ...
                                  <span className="flex-1">{protocol.title}</span>
                                  {protocol.file_url && (
                              ...
                              {experiment.protocols.length === 0 && (
                                <p className="text-xs text-gray-500">No protocols uploaded</p>
                              )}
                            </div>
                          </div>
